...
public class SemanticSignifierFileDao implements SemanticSignifierDao {
    ...
    public SemanticSignifierFileDao(ResourceUtil resourceUtil, Path resource) {
        cache = new HashMap<>();
        restConfigUtil = new RestConfigUtil(); // TODO replace with an injected version
        log.info("Loading resource: " + resource);
        List<SemanticSignifier> sss = restConfigUtil.unmarshalSemanticSignifiers(
                resourceUtil.getResourceAsStream(resource.toString()));
