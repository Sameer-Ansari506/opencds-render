...
public class SupportingDataFileDao implements SupportingDataDao {
    ...
    public SupportingDataFileDao(ResourceUtil resourceUtil, Path path) {
        cache = new HashMap<>();
        restConfigUtil = new RestConfigUtil(); // TODO replace with an injected
        List<String> resources = resourceUtil.findFiles(path.toFile(), false);
        for (String resource : resources) {
            log.info("Loading Resource: " + resource);
            cacheSupportingData(() -> resourceUtil.getResourceAsStream(resource));
        }
    }

    private void cacheSupportingData(Supplier<InputStream> supplier) {
        Optional.ofNullable(supplier.get())
                .map(restConfigUtil::unmarshalSupportingData)
                .ifPresentOrElse(
                        sd -> cache.put(new SDKey(sd.getKMId(), sd.getIdentifier()), sd),
                        () -> cacheSupportingDataList(supplier));
    }

    private void cacheSupportingDataList(Supplier<InputStream> supplier) {
