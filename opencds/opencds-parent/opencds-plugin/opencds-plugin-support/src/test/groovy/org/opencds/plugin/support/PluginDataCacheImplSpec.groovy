...
class PluginDataCacheImplSpec extends Specification {
    ...
    def "test caching of a given key"() {
        ...
        then:
        cache.cache.cache.get(PluginDataCacheImpl.PLUGIN_DATA).keySet().size() == 1
    }
    ...
    def "test caching two different keys that appear to be the same"() {
        ...
        when:
        int first = cache.cache.getCacheKeys(PluginDataCacheImpl.PLUGIN_DATA).size()
        SupportingData sd2 = create()
        println " >>> $sd2"
        Object obj2 = cache.get(sd2)
        int second = cache.cache.getCacheKeys(PluginDataCacheImpl.PLUGIN_DATA).size()
    ...
    private static SupportingData create() {
        return create(new Date())
    }

    private static SupportingData create(Date date) {
        return SupportingData.create(
                ...
                }] as Supplier<SupportingDataPackage>)
    }
}
