...
public class PluginProcessor {
    ...
    public static <D> void preProcess(KnowledgeRepository knowledgeRepository, KnowledgeModule knowledgeModule,
                                      Map<String, SupportingData> supportingData, EvaluationContext context) {
    ...
    private static void applyPreProcessPlugin(KnowledgeRepository knowledgeRepository, EvaluationContext context,
        ...
        var opencdsPlugin = knowledgeRepository.getPluginPackageService()
                .load(tuple.getLeft());
        opencdsPlugin.execute(PreProcessPluginContextImpl.createPreProcessPluginContext(
                filterSupportingData(tuple, supportingData),
    ...
    private static void applyPostProcessPlugin(KnowledgeRepository knowledgeRepository, EvaluationContext context,
        ...
        var opencdsPlugin = knowledgeRepository.getPluginPackageService()
                .load(tuple.getLeft());
        opencdsPlugin.execute(PostProcessPluginContextImpl.createPostProcessPluginContext(
                filterSupportingData(tuple, supportingData),
                knowledgeRepository.getPluginDataCache(tuple.getLeft()),
                context.getAllFactLists(),
                context.getNamedObjects(),
    ...
    private static Map<String, SupportingData> filter(String key,
        ...
        return allSupportingData.get(key).stream().map(Entry::getValue)
